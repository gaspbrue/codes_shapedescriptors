...
from typing import Callable, Dict, List, BinaryIO, Match, Pattern, Tuple, Union, Optional
...
import collections.abc as container_abcs# BMI
#int_classes = int
...
def get_loaders(args, data_folder: str,
        ...
        #list_folders_list = eval("'data/TOY100'+[('gt', gt_transform, True)]")#args.folders)
        #        if depth(list_folders_list) == 1:  # For compatibility reasons, avoid changing all the previous configuration files BMI
        ...
        print()
        train_loaders = []
        for i, (train_topfolder, bounds_generators) in \
                enumerate(zip(args.training_folders, list_bounds_generators)):# , list_folders_list before bounds BMI

                #folders, trans, are_hots = zip('gt', 'gt_transform', True)#*folders_list BMI
                folders, trans, are_hots = ['gt'], [png_transform_npy], [True]#*folders_list BMI
...
class SliceDataset(Dataset):
    def __init__(self, subset, root_dir, n_class = 2,
                 losses_list = None,transform=None,
        ...
        self.losses_list: list =losses_list
    ...
    def __getitem__(self, index) -> Dict[str, Union[Tensor, int, str]]:
        ...
        if(0):#OLD BOUNDS
                ...
                for losses in self.losses_list:
                        ...
                        if bounds_name is None:
                                bounds_generators.append(lambda *a: torch.zeros(self.n_class, 1, 2))
                                continue
                        bounds_class = getattr(__import__('bounds'), bounds_name)
                        bounds_generators.append(bounds_class(C=self.n_class, fn=fn, **bounds_params))

                bounds: list[Tensor]
                if(1):
                        bounds = [f(img, mask) for f in bounds_generators]
                else:#DEBUG
                        pdb.set_trace()
                        bounds = []
                        for f in bounds_generators:
                                toto = f(img, mask) 
                                bounds.append()
                #for i in range(len(bounds)):
                #        bounds[i] = torch.unsqueeze(bounds[i],dim=0)
                #print(bounds[0].shape)
        return {"images": img,
                "gt": mask,
                "filenames": str(img_path),
                "true_size": true_size,
                "bounds": bounds}

class SliceDatasetORI(Dataset):
