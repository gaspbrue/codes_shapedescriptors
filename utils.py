...
def find_size_bounding_box(t: Tensor) -> Tuple[Tensor, Tensor]:
    ...
    w_max_3 = torch.topk(distance_horizontale_ponderee.view(-1), k=3)[0][-1]
    w_min_3 = abs(torch.topk(distance_horizontale_ponderee.view(-1), k=3, largest=False)[0][-1])
    ...
    h_max_3 = torch.topk(distance_verticale_ponderee.view(-1), k=3)[0][-1]
    h_min_3 = abs(torch.topk(distance_verticale_ponderee.view(-1), k=3, largest=False)[0][-1])

 
    h_pred = h_max_3 + h_min_3
    w_pred = w_max_3 + w_min_3
...
def diff_size_bounding_box(t, h, w):
    h_pred, w_pred = find_size_bounding_box(t)
    L = abs(h_pred*w_pred - h*w)
    L_tensor_good = L.view(1,1,1)
    return L_tensor_good

def diff_size_bounding_box_max(t, h, w):
    h_pred, w_pred = find_size_bounding_box(t)
    L = h_pred*w_pred - h*w
    L_tensor_good = L.view(1,1,1)
    return L_tensor_good

def diff_size_bounding_box_min(t, h, w):
    h_pred, w_pred = find_size_bounding_box(t)
    L = h*w - h_pred*w_pred 
    L_tensor_good = L.view(1,1,1)
    return L_tensor_good
