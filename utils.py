...
def soft_size_inside_box(a):
    x1, y1 = 128 - 32, 128 - 32
    x2, y2 = 128 + 32, 128 + 32
    # Sélectionner la région de l'image qui correspond à la bounding box
    box_region = a[:,:, x1:x2, y1:y2]
    box_sum = torch.sum(box_region, dim=(2,3))
    # Ajouter une dimension pour avoir la même forme que l'entrée
    box_sum = box_sum[..., None]
    return box_sum

def soft_size_outside_box(a):
    return (soft_size(a) - soft_size_inside_box(a))
...
"""
def soft_eccentricity(t):
    # Calculer le spectre du tenseur
   eigvals, _ = np.linalg.eig(t)

   # Trier les valeurs propres dans l'ordre décroissant
   eigvals = np.sort(eigvals)[::-1]

   # Calculer l'excentricité
   e = np.sqrt(1 - (eigvals[1] / eigvals[0]) ** 2)
...
def save_images(segs: Tensor, names: Iterable[str], root: Path) -> None:
    for seg, name in zip(segs, names):
        ...
        if len(seg.shape) == 2:
            imsave(str(save_path), seg.detach().cpu().numpy().astype(np.uint8)) # DO NOT NORMALIZE
