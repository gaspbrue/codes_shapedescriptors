...
from utils import save_images_ancien
...
def setup(args, n_class: int) -> Tuple[Any, Any, Any, list[list[Callable]], list[list[float]], Callable]:
    ...
    return net, optimizer, device, loss_fns, loss_weights, scheduler, train_loader, val_loader, e_loss, hwBB_loss,ratiohwBB_loss,sizewgt_loss
...
def do_epoch(mode: str, net: Any, device: Any, loader: DataLoader, epc: int,
             loss_fns: list[Callable], loss_weights: list[float],e_loss,hwBB_loss,ratiohwBB_loss,sizewgt_loss, K: int,
    ...
    for i, data in tq_iter:
        ...
        ziped = zip(loss_fns,  loss_weights, filenames)
        ...
        if(0):
            ...
            for j in range(len(loss_fns)):#for loss_fn, w, bound,filenames in zip
                f=loss_fns[j]
                toto = f(pred_probs, filenames)
                toto = loss_weights[j] * toto
                losses.append(toto)
        

        if (args.e):
            x_min,y_min,x_max,y_max = 50,70,210,220
            box = [x_min,y_min,x_max,y_max]
            toto_e = e_loss(pred_probs, box) 
            toto_e = toto_e * 0.00005
            losses.append(toto_e)
        ...
        if (args.sizewgt):
            bounds = torch.tensor([[[1800,2300]]])
            toto_sizewgt = sizewgt_loss(pred_probs,bounds)
            losses.append(toto_sizewgt)
        ...
        index = 0
        if(args.e):
            loss_log[index] = toto_e
            index +=1
        if(args.hwBB):
            loss_log[index] = toto_hwBB
            index +=1
        if(args.ratiohwBB):
            loss_log[index] = toto_ratiohwBB
            index +=1
        if(args.sizewgt):
            loss_log[index] = toto_sizewgt
            index +=1
        ...
        if savedir:
            with warnings.catch_warnings():
                ...
                if(epc>300000000000000000000000):
                    ...
                    save_images_ancien(predicted_class, 
                                data["names"], 
                                savedir / f"iter{epc:03d}" / "predicted_class_ancien" )
...
def run(args: argparse.Namespace) -> dict[str, Tensor]:
    ...
    net, optimizer, device, loss_fns, loss_weights, scheduler,train_loader,val_loader,e_loss,hwBB_loss,ratiohwBB_loss,sizewgt_loss = setup(args, n_class)
    ...
    for i in range(n_epoch):
        ...
        tra_metrics = do_epoch("train", net, device, train_loader, i,
                               loss_fns, loss_weights,e_loss, hwBB_loss,ratiohwBB_loss,sizewgt_loss, n_class,
        ...
        with torch.no_grad():
            val_metrics = do_epoch("val", net, device, val_loader, i,
                                   loss_fns,
                                   loss_weights,
                                   e_loss,hwBB_loss,ratiohwBB_loss,sizewgt_loss,
                                   n_class,
                                   savedir=savedir,
                                   b_ce=args.b_ce,
                                   ob_ce=args.ob_ce,
...
def setArgParam(args):
    ...
    args.b_ce,args.ob_ce,args.S,args.C,args.D,args.L,args.CP,args.e,args.hwBB,args.ratiohwBB,args.sizewgt = False,False,False,False,False,False,False,False,False,False,False
