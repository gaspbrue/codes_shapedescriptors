...
class BoundingBoxLoss():
    ...
    def __call__(self, probs: Tensor) -> Tensor:
        ...
        size_outside: Tensor = soft_size_outside_box(probs)
        assert size_outside.shape == (B, 2, 1), size_outside.shape
        
        z: Tensor = cast(Tensor, (size_outside.type(torch.float32)).reshape(B, 2))
