...
class BoundingBoxLoss():
    ...
    def __call__(self, probs: Tensor) -> Tensor:
        ...
        probs_classe = probs[:,self.idc,:,:]

        size_outside: Tensor = soft_size_outside_box(probs_classe)
        
        assert size_outside.shape == (B, 1, 1), size_outside.shape

        z: Tensor = cast(Tensor, (size_outside.type(torch.float32)).reshape(B, 1))
