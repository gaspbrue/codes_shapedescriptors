...
def setup(args, n_class: int) -> Tuple[Any, Any, Any, list[list[Callable]], list[list[float]], Callable]:
    ...
    return net, optimizer, device, loss_fns, loss_weights, scheduler, train_loader, val_loader
...
def do_epoch(mode: str, net: Any, device: Any, loader: DataLoader, epc: int,
             loss_fns: list[Callable], loss_weights: list[float],K: int,
...
def run(args: argparse.Namespace) -> dict[str, Tensor]:
    ...
    net, optimizer, device, loss_fns, loss_weights, scheduler,train_loader,val_loader = setup(args, n_class)
...
def setArgParam(args):
    ...
    args.b_ce,args.ob_ce,args.S,args.C,args.D,args.L,args.CP = False,False,False,False,False,False,False
