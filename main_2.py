...
def setup(args, n_class: int) -> Tuple[Any, Any, Any, list[list[Callable]], list[list[float]], Callable]:
    ...
    bb_class = getattr(__import__('losses'), 'BoundingBoxLoss1')
    bb_loss = bb_class(nd=nd,** {'idc': [1], 't': 1})
    
    sizebb_class = getattr(__import__('losses'), 'BoundingBoxLoss2')
    sizebb_loss = sizebb_class(nd=nd,** {'idc' : [1], 't' : 1})
    
    
    print("END SETUP")

    return net, optimizer, device, loss_fns, loss_weights, scheduler, train_loader, val_loader, bb_loss, sizebb_loss
...
def do_epoch(mode: str, net: Any, device: Any, loader: DataLoader, epc: int,
             loss_fns: list[Callable], loss_weights: list[float],bb_loss,sizebb_loss, K: int,
    ...
    for i, data in tq_iter:
        ...
        if (args.BB):
            box = [96,96,160,160]     #xmin,ymin,xmax,ymax
            toto = bb_loss(pred_probs, box) 
            losses.append(toto)
        

        if (args.sizeBB):
            h,w = 138,102
            toto = sizebb_loss(pred_probs,h,w) 
            losses.append(toto)
...
def run(args: argparse.Namespace) -> dict[str, Tensor]:
    ...
    net, optimizer, device, loss_fns, loss_weights, scheduler,train_loader,val_loader,bb_loss,sizebb_loss = setup(args, n_class)
    ...
    for i in range(n_epoch):
        ...
        tra_metrics = do_epoch("train", net, device, train_loader, i,
                               loss_fns, loss_weights,bb_loss, sizebb_loss, n_class,
...
def setArgParam(args):
    ...
    args.BB,args.b_ce,args.ob_ce,args.S,args.C,args.D,args.L,args.CP,args.sizeBB = False,False,False,False,False,False,False,False,False
